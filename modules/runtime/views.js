(function () {
  ...
  root.createRuntimeViews = function createRuntimeViews(runtime) {
    ...
    `;
      ...
      ${(() => {
        const presets = (runtime.state.personalData.presets || []).filter((p) => p.commandId === command.id);
        const hasPresets = presets.length > 0;
        return runtime.renderCollapsibleSection({
          id: `workspace-presets-${command.id}`,
          title: "预设",
          desc: "保存当前参数为一键加载的预设，支持多个预设自由切换。",
          defaultCollapsed: !hasPresets,
          body: `<div class="gm-helper-form-grid"><div class="gm-helper-field gm-helper-preset-row"><select class="gm-helper-select" data-field="selectedPresetId" data-command-id="${command.id}"><option value="">-- 选择预设 --</option>${presets.map((p) => `<option value="${runtime.escapeHtml(p.id)}" ${ws.selectedPresetId === p.id ? "selected" : ""}>${runtime.escapeHtml(p.name)}</option>`).join("")}</select><div class="gm-helper-button-row"><button type="button" class="gm-helper-button gm-helper-button-secondary" data-action="load-preset" data-command-id="${command.id}" ${!ws.selectedPresetId ? "disabled" : ""}>加载</button><button type="button" class="gm-helper-button gm-helper-button-ghost" data-action="delete-preset" data-command-id="${command.id}" ${!ws.selectedPresetId ? "disabled" : ""}>删除</button></div></div><div class="gm-helper-field gm-helper-preset-row"><input class="gm-helper-input" data-field="presetDraftName" data-command-id="${command.id}" placeholder="输入预设名称" value="${runtime.escapeHtml(ws.presetDraftName || "")}" /><button type="button" class="gm-helper-button gm-helper-button-accent" data-action="save-preset" data-command-id="${command.id}" ${!String(ws.presetDraftName || "").trim() ? "disabled" : ""}>保存当前参数</button></div></div>`
        });
      })()}
      ...
      ${runtime.renderCollapsibleSection({
        ...
        body: `${(() => { const hasOutput = Boolean(String(ws.output || "").trim()); return "<textarea class=\"gm-helper-textarea gm-helper-output\" data-field=\"workspace-output\" data-command-id=\"" + command.id + "\" placeholder=\"点击“生成命令”后显示在这里\">" + runtime.escapeHtml(ws.output) + "</textarea><div class=\"gm-helper-button-row\"><button type=\"button\" class=\"gm-helper-button gm-helper-button-accent\" data-action=\"generate\" data-command-id=\"" + command.id + "\">生成命令</button><button type=\"button\" class=\"gm-helper-button gm-helper-button-secondary\" data-action=\"copy\" data-command-id=\"" + command.id + "\" " + (hasOutput ? "" : "disabled") + ">复制</button><button type=\"button\" class=\"gm-helper-button gm-helper-button-secondary\" data-action=\"fill\" data-command-id=\"" + command.id + "\" " + (hasOutput ? "" : "disabled") + ">填入后台</button><button type=\"button\" class=\"gm-helper-button gm-helper-button-secondary\" data-action=\"append\" data-command-id=\"" + command.id + "\" " + (hasOutput ? "" : "disabled") + ">追加到后台</button><button type=\"button\" class=\"gm-helper-button gm-helper-button-ghost\" data-action=\"clear-output\" data-command-id=\"" + command.id + "\" " + (hasOutput ? "" : "disabled") + ">清空结果</button></div>"; })()}${(() => { const pc = runtime.state.ui.pendingConfirm; return pc && pc.commandId === command.id ? "<div class=\"gm-helper-confirm-bar\"><div class=\"gm-helper-confirm-msg\">" + runtime.escapeHtml(pc.message) + "</div><div class=\"gm-helper-button-row\"><button type=\"button\" class=\"gm-helper-button gm-helper-button-danger\" data-action=\"confirm-pending-proceed\" data-command-id=\"" + command.id + "\">" + (pc.appendMode ? "确认追加" : pc.action === "switch-command" ? "确认切换" : "确认") + "</button><button type=\"button\" class=\"gm-helper-button gm-helper-button-ghost\" data-action=\"confirm-pending-cancel\" data-command-id=\"" + command.id + "\">取消</button></div></div>" : ""; })()}`
      })}
    `;
      }
    };
