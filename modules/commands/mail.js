(function () {
  ...
  root.createMailCommands = function createMailCommands(h) {
    const { cmd, param, num, uidParam, buildSimple, buildSendMail, buildSendTemplateMail, buildSendCustomMail, buildRevokeMail } = h;
    return [
      cmd("send_mail", "发送模板邮件", "send_mail", "mail", ["邮件", "邮箱", "发信", "补偿邮件"], ["邮件", "发放", "奖励"], "给玩家发送带附件的模板邮件。", "补偿发放、模板奖励验证。", "normal", [uidParam(), param("mailTemplateId", "邮件模板ID", "text", { required: true, placeholder: "输入邮件标题/ID筛选，可多选", inputMode: "picker_multi", optionSource: "mail" }), param("itemId", "附件道具ID", "text", { required: true, placeholder: "按分类选择后再搜索，可多选", inputMode: "picker_multi", optionSource: "items", pickerFilters: { defaultScope: "in-game", allowCrossScope: true } }), num("count", "附件数量", "例如 1", { defaultValue: "1" })], buildSendMail),
      cmd("send_template_mail", "模板邮件多附件", "send_template_mail", "mail", ["模板邮件多附件", "多物品邮件", "一封邮件多道具"], ["邮件", "发放", "多附件"], "发送模板邮件并一次附带多个道具项。", "按模板统一补偿多件物品。", "normal", [uidParam(), param("mailTemplateId", "邮件模板ID", "text", { required: true, placeholder: "输入邮件标题/ID筛选", inputMode: "picker_single", optionSource: "mail" }), param("itemList", "道具列表", "textarea", { required: true, inputMode: "picker_mail_item_list", optionSource: "items", pickerFilters: { defaultScope: "in-game", allowCrossScope: true }, helper: "先搜索并勾选道具，再加入附件列表。每项最终格式为 itemid/num/bind。" })], buildSendTemplateMail),
